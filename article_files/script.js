;(function(window){
	...
	window.FTRForm.prototype = {
		...
		validate : function(ajax_post)
		{
			...
			// /bitrix/templates/babytoday/components/bitrix/forum.topic.reviews/babytoday/form.php
			//если есть значит не авторизованные пользователь
			if (typeof FORUM_USER !== 'undefined' )
			{				
				// не зареган				
				if(FORUM_USER.status === true)
				{
					//открываем попап
					$('.vhod_klub a').click();
					//меняем на вкладку
					setTimeout(function(){ $('.prettytabs li[class!=" actab"]').click(); },200);
...
console.info('GET', result);
	...
	window.PostFormAjaxStatus = function(status)
	{
		...
		$('.reviews-reply-form').before(statusDIV);
		//$('.reviews-reply-form').hide();
